...
수집 로직은 콘솔 버전(이주희_공공급식조달자동화.py)의 코드를 이 파일 안에 그대로 옮겨 넣었고,
화면만 웹으로 바꾼 것이다. 그래서 이 파일 하나만 있으면 돌아간다(배포 저장소에도 이 파일만 올린다). 브라우저만 있으면 되니까 남에게 잠깐 보여주기 좋다.
...
# 서버(Render)에서는 브라우저를 띄울 화면이 없으므로 내 PC에서 돌릴 때만 연다.
...
def start_collect(inst_nm, year, item_keyword, session):
    ...
    item_keyword = (item_keyword or "").strip() or DEFAULT_ITEM_KEYWORD
    year = int(year)
...
def year_hint(year):
    periods = build_periods_for_year(int(year))
    if not periods:
